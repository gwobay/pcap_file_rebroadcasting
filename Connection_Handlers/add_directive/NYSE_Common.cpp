...
namespace hf_md {
  ...
  bool
  NYSE_Common::init_mcast(const string& line_name, const Parameter& params)
  {
    //const char* where = "NYSE_Common::init_mcast";
    ...
    m_logger->log_printf(Log::INFO, "%s %s: Lines for %s initialized", "NYSE_Common", m_name.c_str(), line_name.c_str());

    return true;
  }

  void
  NYSE_Common::init(const string& name,const Parameter& params)
  {
    MDProvider::init(name, params);
    if(m_supported_exchanges.size() != 1) {
      throw runtime_error("NYSE_Common Parser supports exactly 1 exchange per Provider");
    }
    ...
    if(has_nyse && has_arca) {
      throw runtime_error("NYSE_Common Parser has both NYSE and ARCA feeds shared, one parser can only have 1 parser type");
    }
    if(!(has_nyse || has_arca)) {
      throw runtime_error("NYSE_Common Parser has valid feed type configured");
    }

    if(has_nyse) {
      NYSE_Handler* nh = new NYSE_Handler(&m_md.app());
      m_parser.reset(nh);
      ...
      m_parser.reset(new ARCA_Handler(&m_md.app()));
    }
  ...
  void
  NYSE_Common::establish_retransmission_connections()
  {
    const char* where = "NYSE_Common::establish_retransmission_connections";
  ...
  void
  NYSE_Common::destroy_retransmission_connections()
  {
    //const char* where = "NYSE_Common::destroy_retransmission_connections";
  ...
  void
  NYSE_Common::subscribe_product(const ProductID& id, const ExchangeID& exch,
  ...
  bool
  NYSE_Common::drop_packets(int num_packets)
  {
    if(m_parser) {
  ...
  NYSE_Common::NYSE_Common(MDManager& md) :
    MDProvider(md),
    m_disp(0),
    m_recov_disp(0)
  {
