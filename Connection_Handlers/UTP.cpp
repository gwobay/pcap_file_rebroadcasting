...
namespace hf_md {
  ...
  void
  UTP::init_mcast(const string& line_name, const Parameter& params)
  {
    if(!params.has(line_name)) {
      return;
    }

    UTP_Handler::channel_info_map_t& cim = m_handler->m_channel_info_map;
    ...
    for(vector<Parameter>::const_iterator l = lines.begin(); l != lines.end(); ++l) {
      ...
      cim.push_back(UTP_Handler::channel_info_t(name, context));
      cim.back().init(params);
    }

    m_logger->log_printf(Log::INFO, "%s %s: Lines for %s initialized", "UTP", m_name.c_str(), line_name.c_str());
  }


  void
  UTP::init(const string& name,const Parameter& params)
  {
  ...
  void
  UTP::subscribe_product(const ProductID& id, const ExchangeID& exch,
                            const string& mdSymbol, const string& mdExch)
  {
    m_handler->subscribe_product(id, exch, mdSymbol, mdExch);
  }

  UTP::UTP(MDManager& md) :
    MDProvider(md),
    m_disp(0)
  {
    m_handler.reset(new UTP_Handler(&md.app()));
  }
