#ifndef hf_md_QBT_Simple_Dispatcher_h
#define hf_md_QBT_Simple_Dispatcher_h
...
namespace hf_cwtest {
  ...
  typedef int (*UDP_READER)(int fd, const char* data_buf, size_t len);

//  static char UDP_BOUNDARY=1;

  class Simple_Dispatcher : public MDDispatcher {
  public:
    struct Context {
      Handler* handler;
    ...
    void set_external_start(){m_started=true;}
    void set_udp_reader(int fd, UDP_READER reader);

    explicit Simple_Dispatcher();
...
#endif /* ifndef hf_md_QBT_Simple_Dispatcher_h */
