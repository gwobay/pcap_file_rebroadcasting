...
void addTcpPorts(Parameter& aParam, size_t ttChannel, vector<sockaddr_in>& srv_tcp_list){
	int port_base=stoi(aParam["TCP_port"].getString());
      for (size_t i=0; i<ttChannel; i++) {
	      srv_tcp_list.push_back(sockaddr_in());
	      sockaddr_in snapshot_addr;
	      fill_sockaddr_in(srv_tcp_list.back(), "127.0.0.1", port_base+i);
...
cout << " wait for 2 sec to start" << endl;
std::this_thread::sleep_for(std::chrono::milliseconds(2000));
	...
	EPoll_Dispatcher dispatcher;
	dispatcher.init("EpollDisp", &md, aParam);
	...
	for (size_t c=0; c<ttChannel; c++){
		...
		newC.timeout=msec(1000);
	}
	std::this_thread::sleep_for(std::chrono::milliseconds(1000));
	/*connectTcpPorts(aParam, ttChannel);*/
	vector<sockaddr_in> addr_lists;
	//addTcpPorts(aParam, ttChannel, addr_lists);
	int port_base=stoi(aParam["TCP_port"].getString());
	for (size_t c=0; c<ttChannel; c++){
		const string name=bName+std::to_string(c);
		size_t ch_id=c;
		hf_core::channel_info& newC=channels[c];
		newC.snapshot_socket.fd=-1;//TCP_fds[c];
		fill_sockaddr_in(newC.snapshot_addr, "127.0.0.1", port_base+c);
	//memcpy(&(newC.snapshot_addr), &addr_lists[c], sizeof(sockaddr_in));
		//dispatcher.add_tcp_fd(hdr, name, ch_id, TCP_fds[c], 56000);
