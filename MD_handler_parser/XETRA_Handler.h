#ifndef hf_core_Data_XETRA_Handler_h
#define hf_core_Data_XETRA_Handler_h
...
#include <Data/Xetra_Fast_Decoder.h>
#include <set>
...
namespace hf_core {
  ...
  struct XETRA_Channel_Type
  {
    ...
    static Enum from_string(string s)
    {
      ...
      {
      cout << Time::current_time().to_string() << " - XETRA_Handler.h: invalid XETRA_Channel_Type string " << s << endl;
        throw new exception;
      }
    }
  };
  ...
  struct DeltaMessage
  {
     ...
     DeltaMessage(uint8_t* buf_, size_t len_, uint32_t template_id_, uint32_t seq_num_) :
       len(len_),
  ...
  public:
    ...
    DeltaMessageQueue(size_t max_size)
      : m_max_size(max_size)
     {
     }
     ...
     {
       m_queue.push(new DeltaMessage(buf, len, template_id, seq_num));
     ...
     void Pop()
     {
       DeltaMessage* front = m_queue.front();
       delete front;
       m_queue.pop();
     }
  ...
  struct Xetra_Security_Definition
  {
    Xetra_Security_Definition()
    ...
    bool needs_snapshot;
    DeltaMessageQueue delta_message_queue;
    ...
    uint32_t isix;
    Price_Point m_bids[61]; // 3 * max max_depth + 1
    Price_Point m_offers[61];
    int m_num_bids;
    int m_num_offers;
  ...
  class XETRA_Channel_Handler
  {
  public:
    XETRA_Channel_Handler(int channel_id,
    ...
    void clear_book(Xetra_Security_Definition& sec_def, Time& exch_time);

    template<typename T> void show_snapshot(T& msg, Xetra_Security_Definition& sec_def);
    template<typename T> void apply_snapshot(T& msg, Xetra_Security_Definition& sec_def);
    template<typename T> void process_delta(T& delta, Xetra_Security_Definition& sec_def);

    void handle_state_update(uint32_t state, Xetra_Security_Definition& sec_def);
  ...
  private:
    ...
    void send_quote_update(Xetra_Security_Definition& sec_def);
    ...
    Xetra_Fast_Decoder m_fast_decoder;
  };


  struct XETRA_Context_Info
  {
    XETRA_Channel_Handler* m_channel_handler;
    channel_info* m_channel_info;
    XETRA_Channel_Type::Enum m_channel_type;
    ...
    size_t handle(size_t context, const char* bufC, size_t len)
    {
       ...
       {
         case XETRA_Channel_Type::DELTA:
           {
             return m_channel_handler->handle_delta(context, buf, len);
           }
           break;
         case XETRA_Channel_Type::SNAPSHOT:
           {
             return m_channel_handler->handle_snapshot(context, buf, len);
           }
           break;
         case XETRA_Channel_Type::INSTRUMENT:
           {
             cout << "XETRA_Context_Info::handle: channel type=instrument.  Shouldn't happen since instruments are handled by the XETRA_Handler directly" << endl;
             throw new std::exception();
           }
           break;
         case XETRA_Channel_Type::TRADES:
           {
    ...
    void init(XETRA_Channel_Handler* channel_handler_, channel_info* channel_info_, XETRA_Channel_Type::Enum channel_type_)
    {
  ...
  class XETRA_Handler : public Handler {
  public:
    // Handler functions
    ...
    XETRA_Handler(Application* app);
    virtual ~XETRA_Handler();

  private:
    typedef tbb::spin_mutex mutex_t;

    XETRA_Channel_Handler* register_channel(int channel_id)
    {
      hash_map<int, XETRA_Channel_Handler*>::iterator it = m_channel_handlers.find(channel_id);
      if(it == m_channel_handlers.end())
      {
        m_channel_handlers[channel_id] = new XETRA_Channel_Handler(channel_id, m_order_book, m_exch, m_exch_rule_global, m_feed_rules_global, m_subscribers, m_provider, m_prod_map_global, m_security_definitions_by_isix, m_qbdf_builder, m_qbdf_exch_char_global, m_security_status, m_logger, this);
      }
    ...
    vector<XETRA_Context_Info> m_context_info;
    ...
    Xetra_Fast_Decoder m_fast_instrument_decoder;
    ...
    Xetra_Security_Definition m_security_definitions_by_isix[MAX_XETRA_ISIX];
    ...
    hash_map<int, XETRA_Channel_Handler*> m_channel_handlers; 
...
#endif /* ifndef hf_core_Data_XETRA_Handler_h */
