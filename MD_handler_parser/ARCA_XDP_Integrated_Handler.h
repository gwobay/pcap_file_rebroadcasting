...
namespace hf_core {
  ...
  class ARCA_XDP_Integrated_Handler : public Handler {
  public:
    ...
    struct symbol_info {
      ...
      uint32_t  last_trade_id_seen;
      uint64_t  last_trade_order_id;
      char      last_trade_side;
      bool      last_order_executed_at_different_price;     
 
      symbol_info() :
        price_scale_code(-1), matching_engine(0), last_trade_id_seen(0)
      { }
    };
    ...
    ARCA_XDP_Integrated_Handler(Application* app);
    virtual ~ARCA_XDP_Integrated_Handler();
  ...
  namespace ARCA_XDP_Integrated_structs {
...
// Response Messages
	...
	struct trade_message {
		...
		uint8_t  liquidity_indicator_flag;
		uint32_t ask_price;
		uint32_t ask_volume;
		uint32_t bid_price;
		uint32_t bid_volume;
	...
	// Msg Type 221
	struct trade_cancel_or_bust_message {
		uint32_t source_time;
		uint32_t source_time_ns;
		uint32_t symbol_index;
		uint32_t symbol_seq_num;
