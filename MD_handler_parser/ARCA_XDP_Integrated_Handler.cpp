...
#include <Data/ARCA_XDP_Integrated_Handler.h>
...
#define CLASS_NAME "ARCA_XDP_Integrated_Handler"
...
namespace hf_core {
  ...
  static FactoryRegistrar1<std::string, Handler, Application*, ARCA_XDP_Integrated_Handler> r1("ARCA_XDP_Integrated_Handler");
  
  using namespace ARCA_XDP_Integrated_structs;
...
#define ORDER_ID(obj)                                   \
  static_cast<uint64_t>(obj.order_id) + ((uint64_t)obj.symbol_index << 32)
  
#define NEW_ORDER_ID(obj)                               \
  static_cast<uint64_t>(obj.new_order_id) + ((uint64_t)obj.symbol_index << 32)
...
#define GET_SYMBOL(msg)                                                \
  ...
  size_t
  ARCA_XDP_Integrated_Handler::parse(size_t context, const char* buf, size_t len)
  {
    ...
    ARCA_XDP_Integrated_Handler::parse2(context, buf, len);
  ...
  size_t
  ARCA_XDP_Integrated_Handler::parse2(size_t context, const char* orig_buf, size_t len)
  {
    ...
    for(int msg_idx = 0; msg_idx < num_msgs; ++msg_idx) {
      ...
      if(buf > buf_end) {
	send_alert("ARCA_XDP_Integrated: %s overran buffer length, len=%zd, msg_type=%d, msg_size=%d", m_name.c_str(), len, msg_type, msg_size);
	break;
      }
      ...
      case 33: // trading session change
	{
	  ...
	  if (m_order_book) {
		...
		break;
	      ...
	      Time t = m_time_references[msg.symbol_index] + Time_Duration(msg.source_time_ns);
	  ...
	  if(id != Product::invalid_product_id) {
		...
		if(inside_changed) {
		  ...
		  send_quote_update();
                ...
                bool core_eligible = (msg.trade_session & (0x01 << 1));
                if(!core_eligible) {
                  MDOrderBookHandler::mutex_t::scoped_lock lock(m_order_book->m_mutex);
                ...
                bool late_eligible = (msg.trade_session & (0x01 << 2));
                if(!late_eligible) {
                  MDOrderBookHandler::mutex_t::scoped_lock lock(m_order_book->m_mutex);
                  MDOrder* order = m_order_book->unlocked_find_order(order_id);
                  if(order) {
	  ...
	  if(id != Product::invalid_product_id) {
		...
		MDOrder* order = m_order_book->unlocked_find_order(order_id);
		  ...
		  if(price == order->price) { 
		    inside_changed = m_order_book->unlocked_modify_order(t, order, size); // ToDo: is this correct?
		  } else {
	  ...
	  const execution_message& msg = reinterpret_cast<const execution_message&>(*msg_buf);
	    ...
	    if (m_order_book) {
	      ...
	      uint64_t order_id = ORDER_ID(msg);
              ...
              si->last_trade_id_seen = msg.trade_id;
              si->last_trade_order_id = ORDER_ID(msg);
              si->last_trade_side = side;
              si->last_order_executed_at_different_price = (order && order->price != price);
      ...
      case 105: // Imbalance Message
	{
	  ...
	  if(id != Product::invalid_product_id) {
	    ...
	    MDPrice ref_price = get_int_price(msg.indicative_match_price, si->price_scale_code);
            MDPrice near_price = get_int_price(msg.indicative_match_price, si->price_scale_code);
            MDPrice far_price = get_int_price(msg.indicative_match_price, si->price_scale_code);
	...
	break;
	  ...
	  if(id != Product::invalid_product_id) {
            ...
            m_trade.m_trade_qualifier[0] = msg.trade_cond_1;
            m_trade.m_trade_qualifier[1] = msg.trade_cond_2;
            m_trade.m_trade_qualifier[2] = msg.trade_cond_3;
            m_trade.m_trade_qualifier[3] = msg.trade_cond_4;
           
            if(si->last_trade_id_seen != msg.trade_id) {
              m_trade.set_exec_flag(TradeUpdate::exec_flag_trade);
              m_trade.set_side('U');
              if(!m_qbdf_builder) {
                send_trade_update();
              } else {
                char cond[5] = { msg.trade_cond_1, msg.trade_cond_2, msg.trade_cond_3, msg.trade_cond_4, '\0'}; 
                gen_qbdf_trade_micros(m_qbdf_builder, id, m_micros_recv_time, m_micros_exch_time_delta,
                                      m_exch_char, 'U', '0', cond, msg.volume, price);
              }
            } else {
              m_trade.set_side(si->last_trade_side);
              if(!m_qbdf_builder) {
                if(si->last_order_executed_at_different_price) {
                ...
                char cond[5] = { msg.trade_cond_1, msg.trade_cond_2, msg.trade_cond_3, msg.trade_cond_4, '\0'}; 
                gen_qbdf_order_exec_cond_micros(m_qbdf_builder, id, si->last_trade_order_id,
                                           m_micros_recv_time, m_micros_exch_time_delta,
                                           m_exch_char, m_trade.side(), cond,
                                           msg.volume, price);
              }
            }
      ...
      case 221:
        {
          const trade_cancel_or_bust_message& msg = reinterpret_cast<const trade_cancel_or_bust_message&>(*msg_buf);
          ProductID id = m_symbol_to_product_id.find_uint64(msg.symbol_index); \
          if (m_qbdf_builder) {
            if(id != Product::invalid_product_id) {
              Time t = m_time_references[msg.symbol_index] + Time_Duration(msg.source_time_ns);
              ...
              gen_qbdf_cancel_correct(m_qbdf_builder, id,
                                      ...
                                      0, "", "CANCEL", msg.original_trade_id,
                                      0, 0, "", 0, 0, "");
            }
          }
        }
	  ...
	  case 222:
	  {
		  ...
		  if (m_qbdf_builder) {
			  if (id != Product::invalid_product_id) {
				  ...
				  // ToDo: generate proper correct
				  /*
				  gen_qbdf_cancel_correct(m_qbdf_builder, id,
					  m_micros_exch_time / Time_Constants::micros_per_mili,
					  0, "", "CORRECT", msg.original_trade_id,
					  0, 0, "", 0, 0, "");
				  */
			  }
		  }
  ...
  size_t
  ARCA_XDP_Integrated_Handler::dump(ILogger& log, Dump_Filters& filter, size_t context, const char* buf, size_t packet_len)
  {
    const packet_header& packet = reinterpret_cast<const packet_header&>(*buf);
      ...
      case 100: // Add Order Message
	{
	  const add_order_message& msg = reinterpret_cast<const add_order_message&>(*msg_buf);
          ...
          uint32_t symbol_index = msg.symbol_index;
          log.printf("  add_order source_time_ns:%u symbol_index:%u symbol_seq_num:%u order_id:%u price:%u volume:%u side:%c order_idgtc_indicator:%u trade_session:%u\n",
                     msg.source_time_ns, symbol_index, msg.symbol_seq_num, msg.order_id, msg.price, msg.volume, msg.side, msg.order_idgtc_indicator, msg.trade_session);
	}
	break;
      case 101: // Modify Order Message
	{
	  const modify_order_message& msg = reinterpret_cast<const modify_order_message&>(*msg_buf);
          ...
          uint32_t symbol_index = msg.symbol_index;
          log.printf("  modify_order source_time_ns:%u symbol_index:%u symbol_seq_num:%u order_id:%u price:%u volume:%u order_idgtc_indicator:%u reason_code: %u\n",
                     msg.source_time_ns, symbol_index, msg.symbol_seq_num, msg.order_id, msg.price, msg.volume, msg.order_idgtc_indicator, msg.reason_code);
	}
	break;
      case 102: // Delete Order Message
	{
	  const delete_order_message& msg = reinterpret_cast<const delete_order_message&>(*msg_buf);
          ...
          uint32_t symbol_index = msg.symbol_index;
          log.printf("  delete_order source_time_ns:%u symbol_index:%u symbol_seq_num:%u order_id:%u side:%c order_idgtc_indicator:%u reason_code:%u\n",
                     msg.source_time_ns, symbol_index, msg.symbol_seq_num, msg.order_id, msg.side, msg.order_idgtc_indicator, msg.reason_code);
	}
	break;
      case 103: // Execution Message
	{
	  const execution_message& msg = reinterpret_cast<const execution_message&>(*msg_buf);
          ...
          uint32_t symbol_index = msg.symbol_index;
          log.printf("  execute_order source_time_ns:%u symbol_index:%u symbol_seq_num:%u order_id:%u price:%u volume:%u order_idgtc_indicator:%u reason_code:%u trade_id:%u\n",
                     msg.source_time_ns, symbol_index, msg.symbol_seq_num, msg.order_id, msg.price, msg.volume, msg.order_idgtc_indicator, msg.reason_code, msg.trade_id);
	}
	break;
      case 105:
	{
	  const imbalance_message& msg = reinterpret_cast<const imbalance_message&>(*msg_buf);
	  log.printf("  imbalance_message source_time_ns:%u symbol_index:%u symbol_seq_num:%u indicative_match_price:%u paired_qty:%u total_imbalance_qty:%d market_imbalance_qty:%d side:%c\n",
		     msg.source_time_ns, msg.symbol_index, msg.symbol_seq_num, msg.indicative_match_price, msg.paired_qty, msg.total_imbalance_qty, msg.market_imbalance_qty, msg.imbalance_side);
	}
	break;
      case 106:
        {
	  const add_order_refresh_message& msg = reinterpret_cast<const add_order_refresh_message&>(*msg_buf);
          ...
          uint32_t symbol_index = msg.symbol_index;
		  log.printf("  add_order_refresh source_time_ns:%u symbol_index:%u symbol_seq_num:%u order_id:%u price:%u volume:%u side:%c order_idgtc_indicator:%u trade_session:%u\n",
			  msg.source_time_ns, symbol_index, msg.symbol_seq_num, msg.order_id, msg.price, msg.volume, msg.side, msg.order_idgtc_indicator, msg.trade_session);
        }
        break;
	  case 107:
	  {
		  ...
		  uint32_t symbol_index = msg.symbol_index;
		  log.printf("  attributed_add_order source_time_ns:%u symbol_index:%u symbol_seq_num:%u order_id:%u price:%u volume:%u side:%c order_idgtc_indicator:%u trade_session:%u firm_id:%.5s\n",
			  msg.source_time_ns, symbol_index, msg.symbol_seq_num, msg.order_id, msg.price, msg.volume, msg.side, msg.order_idgtc_indicator, msg.trade_session, msg.firm_id);
	  }
	  case 108:
	  {
		  const attributed_add_order_refresh_message& msg = reinterpret_cast<const attributed_add_order_refresh_message&>(*msg_buf);
		  uint32_t symbol_index = msg.symbol_index;
		  log.printf("  attributed_add_order_refresh source_time_ns:%u symbol_index:%u symbol_seq_num:%u order_id:%u price:%u volume:%u side:%c order_idgtc_indicator:%u trade_session:%u firm_id:%.5s\n",
			  msg.source_time_ns, symbol_index, msg.symbol_seq_num, msg.order_id, msg.price, msg.volume, msg.side, msg.order_idgtc_indicator, msg.trade_session, msg.firm_id);
	  }
	  case 220:
	  {
		  const trade_message& msg = reinterpret_cast<const trade_message&>(*msg_buf);
		  uint32_t symbol_index = msg.symbol_index;
		  log.printf("  trade_message source_time_ns:%u symbol_index:%u symbol_seq_num:%u trade_id:%u price:%u volume:%u trade_cond_1:%c trade_cond_2:%c trade_cond_3:%c trade_cond_4:%c trade_through_exempt:%c liquidity_indicator_flag:%u ask_price:%u ask_volume:%d bid_price:%u bid_volume:%u\n",
			  msg.source_time_ns, symbol_index, msg.symbol_seq_num, msg.trade_id, msg.price, msg.volume, msg.trade_cond_1, msg.trade_cond_2, msg.trade_cond_3, msg.trade_cond_4, msg.trade_through_exempt, 
			  msg.liquidity_indicator_flag, msg.ask_price, msg.ask_volume, msg.bid_price, msg.bid_volume);
	  }
      case 221:
        {
          const trade_cancel_or_bust_message& msg = reinterpret_cast<const trade_cancel_or_bust_message&>(*msg_buf);
          ...
          uint32_t symbol_index = msg.symbol_index;
          log.printf("  trade_cancel_message source_time_ns:%u symbol_index:%u symbol_seq_num:%u original_trade_id:%u\n",
                     msg.source_time_ns, symbol_index, msg.symbol_seq_num, msg.original_trade_id);
        }
        break;
	  case 222:
	  {
		  const trade_correction_message& msg = reinterpret_cast<const trade_correction_message&>(*msg_buf);
		  uint32_t symbol_index = msg.symbol_index;
		  log.printf("  trade_correction_message source_time_ns:%u symbol_index:%u symbol_seq_num:%u original_trade_id:%u trade_id:%u price:%u volume:%u trade_cond_1:%c trade_cond_2:%c trade_cond_3:%c trade_cond_4:%c trade_through_exempt:%c\n",
			  msg.source_time_ns, symbol_index, msg.symbol_seq_num, msg.original_trade_id, msg.trade_id, msg.price, msg.volume, msg.trade_cond_1, msg.trade_cond_2, msg.trade_cond_3, msg.trade_cond_4, msg.trade_through_exempt);
	  }
  ...
  void
  ARCA_XDP_Integrated_Handler::init(const string& name, ExchangeID exch, const Parameter& params)
  {
	...
	si->price_scale_code = price_scale_code;
        ...
        m_time_references.resize(m_app->pm()->max_product_id() + 1);
      }
    }
  ...
  void
  ARCA_XDP_Integrated_Handler::start()
  {
  ...
  void
  ARCA_XDP_Integrated_Handler::stop()
  {
    for(channel_info_map_t::iterator i = m_channel_info_map.begin(), i_end = m_channel_info_map.end(); i != i_end; ++i) {
      i->clear();
    }
  ...
  void
  ARCA_XDP_Integrated_Handler::send_retransmit_request(channel_info& cib)
  {
    ...
    if(ret < 0) {
      ...
      send_alert("ARCA_XDP_Integrated_Handler::send_retransmit_request %s sendto returned %d - %s", m_name.c_str(), errno, err_msg);
    }
  ...
  void
  ARCA_XDP_Integrated_Handler::send_heartbeat_response(channel_info& ci)
  {
    if(!ci.recovery_socket.connected()) {
      return;
    }
  ...
  size_t
  ARCA_XDP_Integrated_Handler::read(size_t context, const char* buf, size_t len)
  {
    if(len < 2) {
      return 0;
    }
  ...
  void
  ARCA_XDP_Integrated_Handler::add_context_product_id(size_t context, ProductID id)
  {
  ...
  void
  ARCA_XDP_Integrated_Handler::subscribe_product(ProductID id, ExchangeID exch, const string& mdSymbol, const string& mdExch)
  {
    ...
    if(symbol_index == 0) {
      m_logger->log_printf(Log::ERROR, "ARCA_XDP_Integrated_Handler::subscribe_product:  Called for invalid symbol_index '%s', product_id %d",
			   mdSymbol.c_str(), id);
      return;
    }
    ...
    if(si->price_scale_code == -1) {
      m_logger->log_printf(Log::ERROR, "ARCA_XDP_Integrated_Handler::subscribe_product:  Unable to find symbol_index '%s', product_id %d in symbol_info_map (no price_scale_code)",
			   mdSymbol.c_str(), id);
      return;
    }
  ...
  void
  ARCA_XDP_Integrated_Handler::reset(size_t context, const char* msg)
  {
    reset(msg);
    return;
    ...
    m_logger->log_printf(Log::WARN, "ARCA_XDP_Integrated_Handler::reset %s called with %zd products", m_name.c_str(), clear_products.size());
  }

  void
  ARCA_XDP_Integrated_Handler::reset(const char* msg)
  {
    if(m_order_book) {
      if (m_qbdf_builder) {
    ...
    m_logger->log_printf(Log::WARN, "ARCA_XDP_Integrated_Handler::reset called %s -- %s", m_name.c_str(), msg);
  }

  void
  ARCA_XDP_Integrated_Handler::send_quote_update()
  {
  ...
  void
  ARCA_XDP_Integrated_Handler::send_trade_update()
  {
  ...
  ProductID
  ARCA_XDP_Integrated_Handler::product_id_of_symbol_index(uint32_t symbol_index)
  {
    ...
    if(m_qbdf_builder) {
      ...
      if (symbol_id == 0) {
        m_logger->log_printf(Log::ERROR, "%s: unable to create or find id for symbol index [%d]",
                             "ARCA_XDP_Integrated_Handler::product_id_of_symbol", symbol_index);
        m_logger->sync_flush();
        return -1;
      }
  ...
  ARCA_XDP_Integrated_Handler::ARCA_XDP_Integrated_Handler(Application* app) :
    Handler(app, "ARCA_XDP_Integrated"),
  ...
  ARCA_XDP_Integrated_Handler::~ARCA_XDP_Integrated_Handler()
  {
  }
