#ifndef hf_core_Data_TMX_QL2_Handler_h
#define hf_core_Data_TMX_QL2_Handler_h
...
namespace hf_core {
  ...
  class TMX_QL2_Handler : public Handler {
  public:
    // Handler functions
    ...
    TMX_QL2_Handler(Application* app);
    virtual ~TMX_QL2_Handler();

  private:
    typedef tbb::spin_mutex mutex_t;
    ProductID product_id_of_symbol(const char* symbol, int len);

    void parseInnerMessage(const char*buf);
    ...
    inline uint32_t getBoardLotSize(ProductID productId)
    {
      hash_map<ProductID, uint32_t>::iterator it = product_id_to_board_lot_size.find(productId);
      if(it != product_id_to_board_lot_size.end())
      {
        return it->second;
      }
      
      Product product = m_app->pm()->find_by_id(productId);
      double price = product.get_double(Product::idx::adj_close);
      if (price == 0.0)
      {
        price = 100.0; // As last resort just assume it's a 100 CAD product
      }
      if(price < .095)
      {
        product_id_to_board_lot_size[productId] = 1000;
        return 1000;
      }
      if(price < 1.0)
      {
        product_id_to_board_lot_size[productId] = 500;
        return 500;
      }
      product_id_to_board_lot_size[productId] = 100;
      return 100;
    }

    inline bool is_odd_lot(uint32_t size, ProductID productId)
    {
      uint32_t lotSize = getBoardLotSize(productId);
      return (size < lotSize);
    }

    inline bool ignore_due_to_odd_lot(ProductID productId, uint32_t size)
    {
      //return false;
      return is_odd_lot(size, productId) && m_ignore_odd_lots;
    }
    ...
    hash_map<ProductID, uint32_t> product_id_to_board_lot_size;
    ...
    hash_map<ProductID, string> product_id_to_symbol;
    bool m_ignore_odd_lots;
  };

}

#endif /* ifndef hf_core_Data_TMX_QL2_Handler_h */
