...
#include <Data/XETRA_Handler_15.h>
...
namespace hf_core {
  ...
  static FactoryRegistrar1<std::string, Handler, Application*, XETRA_Handler_15> r1("XETRA_Handler_15");
...
#define LOGV(x)       \
  ...
  inline static ProductID product_id_of_symbol(QBDF_Builder* qbdf_builder, hash_map<string, ProductID>& prod_map, string& symbol, LoggerRP& logger)
  {
    if (qbdf_builder) {
      ...
      if (symbol_id == 0) {
        ...
        //                     "XETRA_Handler_15::product_id_of_symbol", symbol.c_str());
        //m_logger->sync_flush();
  ...
  void XETRA_CHannel_Handler_15::send_quote_update(Xetra_Security_Definition_15& sec_def)
  {
  ...
  void XETRA_CHannel_Handler_15::send_trade_update()
  {
  ...
  void XETRA_CHannel_Handler_15::send_auction_update(AuctionUpdate& auctionUpdate)
  {
...
#define SIZE_CHECK(type)                                                \
  ...
  size_t
  XETRA_Handler_15::parse(size_t context, const char* buf, size_t len)
  {
    //cout << Time::current_time().to_string() << " parse: context=" << context << endl;
    const char* where = "XETRA_Handler_15::parse";
...
*/  
  ...
  size_t
  XETRA_Handler_15::parse2(size_t context, const char* packet_buf, size_t packet_len)
  {
    if(m_recorder) {
    ...
    XETRA_Context_Info_15& context_info = m_context_info[context];
    
    try {
      if(context_info.m_channel_type == XETRA_Channel_Type_15::INSTRUMENT) {
      ...
      m_logger->log_printf(Log::ERROR, "XETRA_Handler_15::parse2 Caught exception on context %lu Channel_Type %s  -- %s", context, XETRA_Channel_Type_15::to_string(context_info.m_channel_type), e.what());
    } catch(...) {
      m_logger->log_printf(Log::ERROR, "XETRA_Handler_15::parse2 Caught exception on context %lu Channel_Type %s", context, XETRA_Channel_Type_15::to_string(context_info.m_channel_type));
    }
  ...
  size_t
  XETRA_Handler_15::dump(ILogger& log, Dump_Filters& filter, size_t context, const char* packet_buf, size_t packet_len)
  {
    xetra_15::Xetra_Fast_Decoder fast_decoder;
    ...
    while(buf_ptr < end_ptr) {
      ...
      case 6:
        {
          //xetra_15::InsideMarketSnapshotInformation& snapshot = m_fast_decoder.Parse_InsideMarketSnapshotInformation(buf_ptr);
  ...
  void
  XETRA_Handler_15::reset(const char* msg)
  {
    // ToDo:
    /*
    if(msg != 0 && msg[0] != '\0') {
  ...
  void
  XETRA_Handler_15::reset(size_t context, const char* msg)
  {
    // ToDo:
    /*
    const char* where = "XETRA_Handler_15::reset";
    ...
    m_logger->log_printf(Log::WARN, "XETRA_Handler_15::reset %s called with %zd products", m_name.c_str(), ci.clear_products.size());
  ...
  void
  XETRA_Handler_15::init(const string& name, ExchangeID exch, const Parameter& params)
  {
    ...
    if (exch == ExchangeID::XETR) {
      ...
      m_logger->log_printf(Log::ERROR, "XETRA_Handler_15::init: Unrecognized ExchangeID");
  ...
  void
  XETRA_Handler_15::subscribe_product(ProductID id_,ExchangeID exch_, const string& mdSymbol_,const string& mdExch_)
  {
    mutex_t::scoped_lock lock(m_mutex);
  ...
  XETRA_Handler_15::XETRA_Handler_15(Application* app) :
    Handler(app, "XETRA"),
  ...
  XETRA_Handler_15::~XETRA_Handler_15()
  {
  }

  void XETRA_Handler_15::set_up_channels(const Parameter& params)
  {
    ...
    for(vector<Parameter>::const_iterator l = lines.begin(); l != lines.end(); ++l) {
      ...
      XETRA_Context_Info_15& context_info = m_context_info[context];
      XETRA_CHannel_Handler_15* channel_handler = register_channel(channel_id);
      XETRA_Channel_Type_15::Enum channel_type = XETRA_Channel_Type_15::from_string(channel_type_str);
  ...
  XETRA_CHannel_Handler_15::XETRA_CHannel_Handler_15(int channel_id,
  ...
  uint64_t adjust_to_london_time_15(uint64_t entry_time)
  {
    return entry_time - (60*60*1000000ull);
  }

  template<typename T>
  void XETRA_CHannel_Handler_15::process_delta(T& delta, Xetra_Security_Definition_15& sec_def)
  {
    //cout << "process_delta ****************PACKET************************" << endl;
    FAST_Sequence<xetra_15::InsideMarketDeltaInformation_EntriesDepth_Element, 100>& entriesDepth = delta.EntriesDepth();
    bool inside_changed = false;
    Time exch_time = Time::today() + Time_Duration(static_cast<int64_t>(adjust_to_london_time_15(delta.entryTime())) * 1000);
    ...
    {
      xetra_15::InsideMarketDeltaInformation_EntriesDepth_Element& entry = entriesDepth.entries[i];
...
*/
              ...
              case 1: // CRE (New)
                {
                  ...
                  {
                    memmove(&(sec_def.m_offers[price_level]), &(sec_def.m_offers[price_level - 1]), sizeof(Price_Point_15) * (sec_def.m_num_offers - price_level + 1));
                  }
              ...
              case 3: // DEL (Delete)
                {
                  ...
                  {
                    memmove(&(sec_def.m_offers[price_level - 1]), &(sec_def.m_offers[price_level]), sizeof(Price_Point_15) * (sec_def.m_num_offers - price_level));
                  }
                  --(sec_def.m_num_offers);
                }
                break;
              ...
              case 5: // DEL_THR ( Delete Through. Delete from level 1 to level X)
                {
                  ...
                  if(static_cast<int>(price_level) < sec_def.m_num_offers)
                  {
                    memmove(&(sec_def.m_offers[0]), &(sec_def.m_offers[price_level]), sizeof(Price_Point_15) * num_remaining_levels);
                  }
              ...
              case 1: // CRE (New)
                {
                  ...
                  {
                    memmove(&(sec_def.m_bids[price_level]), &(sec_def.m_bids[price_level - 1]), sizeof(Price_Point_15) * (sec_def.m_num_bids - price_level + 1));
                  }
              ...
              case 3: // DEL (Delete)
                {
                  ...
                  if(sec_def.m_num_bids > static_cast<int>(price_level))
                  {
                    memmove(&(sec_def.m_bids[price_level - 1]), &(sec_def.m_bids[price_level]), sizeof(Price_Point_15) * (sec_def.m_num_bids - price_level));
                  }
                  --(sec_def.m_num_bids);
                }
                break;
              ...
              case 5: // DEL_THR ( Delete Through. Delete from level 1 to level X)
                {
                  ...
                  if(static_cast<int>(price_level) < sec_def.m_num_bids)
                  {
                    memmove(&(sec_def.m_bids[0]), &(sec_def.m_bids[price_level]), sizeof(Price_Point_15) * num_remaining_levels);
                  }
        ...
        case 23: // ASK_PRC_MARKET:
          {
            switch(action)
            {
              case 1: //NEW
              {
                ...
                {
                  memmove(&(sec_def.m_offers[1]), &(sec_def.m_offers[0]), sizeof(Price_Point_15) * sec_def.m_num_offers);
                }
        ...
        case 24: // BID_PRC_MARKET:
          {
            switch(action)
            {
              case 1: //NEW
              {
                ...
                {
                  memmove(&(sec_def.m_bids[1]), &(sec_def.m_bids[0]), sizeof(Price_Point_15) * sec_def.m_num_bids);
                }
    ...
    FAST_Sequence<xetra_15::InsideMarketDeltaInformation_EntriesTrade_Element, 100>& trades = delta.EntriesTrade();
//    if(trades.len > 0)
//    {
...
//    }
    ...
    for(int j = trades.len - 1; j >= 0; --j)
    {
      ...
      xetra_15::InsideMarketDeltaInformation_EntriesTrade_Element& trade = trades.entries[j];
      ...
      switch(entryType)
      {
        ...
        //case 10: // BEST_PRC
          {
            FAST_Sequence<xetra_15::InsideMarketDeltaInformation_EntriesTrade_Element_EntriesTradePrices_Element, 50>& tradePrices = trade.EntriesTradePrices();
            //cout << "Got trade prices. len=" << tradePrices.len << endl;
            for(int i = tradePrices.len - 1; i >= 0; --i)
            {
              xetra_15::InsideMarketDeltaInformation_EntriesTrade_Element_EntriesTradePrices_Element& entry = tradePrices.entries[i];
              ...
              uint64_t entryTime = adjust_to_london_time_15(entry.entryTime());
    ...
    if(delta.auctionGroup().has_value)
    {
      ...
      if(moiInterruption == false && volInterruption == false) // Ignore these
      {
        ...
        FAST_Sequence<xetra_15::InsideMarketDeltaInformation_auctionGroup_Group_EntriesAuction_Element, 50>& entries = delta.auctionGroup().value.EntriesAuction();
        for(size_t i = 0; i < entries.len; ++i)
        {
          xetra_15::InsideMarketDeltaInformation_auctionGroup_Group_EntriesAuction_Element& entry = entries.entries[i];
 ...
 * */
  ...
  void XETRA_CHannel_Handler_15::handle_state_update(uint32_t state, Xetra_Security_Definition_15& sec_def)
  {
  ...
  size_t XETRA_CHannel_Handler_15::handle_delta(size_t context, uint8_t* buf, size_t len)
  {
    ...
    while(buf_ptr < end_ptr)
    {
      ...
      switch(template_id)
      {
        ...
        case 7:
          {
            xetra_15::InsideMarketDeltaInformation& delta = m_fast_decoder.Parse_InsideMarketDeltaInformation(buf_ptr);
...
*/
            ...
            LOGV(cout << "delta entryTime=" << adjust_to_london_time_15(delta.entryTime()) << endl);
            if(isix < MAX_XETRA_ISIX)
            {
              Xetra_Security_Definition_15& sec_def = m_security_definitions_by_isix[isix];
...
*/
                  ...
                  clear_book(sec_def, exch_time);
                  process_delta<xetra_15::InsideMarketDeltaInformation>(delta, sec_def);
                ...
                else if((delta.seqNum() > sec_def.expected_seq_num && sec_def.expected_seq_num > 2))
                {
                  ...
                  {
                    m_logger->log_printf(Log::WARN, "XETRA_Handler_15::gap for isix=%d: expecting %d; got %d", sec_def.isix, sec_def.expected_seq_num, delta.seqNum());
                    Time exch_time = Time::current_time();
                    clear_book(sec_def, exch_time);
                  }
                  sec_def.needs_snapshot = true;
                ...
                m_logger->log_printf(Log::WARN, "Likely sequence number reset. isix=%d seqNum=1 expected=%d. Will get snapshot", isix, sec_def.expected_seq_num);
                  if (sec_def.expected_seq_num > 1)
                  {
                    m_logger->log_printf(Log::WARN, "XETRA_Handler_15::much lower seq num than expected for isix=%d: expecting %d; got %d", sec_def.isix, sec_def.expected_seq_num, delta.seqNum());
                    Time exch_time = Time::current_time();
                    clear_book(sec_def, exch_time);
                  }
        ...
        case 13: 
          {
            xetra_15::InstrumentStateMessage& instrument_state = m_fast_decoder.Parse_InstrumentStateMessage(buf_ptr);
            ...
            if(isix < MAX_XETRA_ISIX)
            {
              ...
              if(sec_def.product_id != Product::invalid_product_id)
              {
                ...
                m_logger->log_printf(Log::INFO, "XETRA: Got xetra_15::InstrumentStateMessage isix=%d state=%d", isix, state);
  ...
  void XETRA_CHannel_Handler_15::qbdf_update_level(ProductID product_id, char side, uint32_t shares, MDPrice md_price, uint32_t num_orders, bool last_in_group, Time& exch_time)
  {
    if(m_qbdf_builder)
    {
  ...
  void XETRA_CHannel_Handler_15::clear_book(Xetra_Security_Definition_15& sec_def, Time& exch_time)
  {
    ...
    if(product_id != Product::invalid_product_id)
    {
      ...
      if(m_qbdf_builder)
      {
        for(int i = 0; i < sec_def.m_num_bids; ++i)
        {
          Price_Point_15& bid = sec_def.m_bids[i];
        ...
        for(int i = 0; i < sec_def.m_num_offers; ++i)
        {
          Price_Point_15& offer = sec_def.m_offers[i];
  ...
  template<typename T>
  void XETRA_CHannel_Handler_15::show_snapshot(T& msg, Xetra_Security_Definition_15& sec_def)
  {
#ifdef CW_DEBUG
    cout << Time::current_time().to_string() << " show_snapshot:" << endl;
#endif
    FAST_Sequence<xetra_15::InsideMarketSnapshotInformation_EntriesDepth_Element, 50>& entriesDepth = msg.EntriesDepth();
    for(size_t i = 0; i < entriesDepth.len; ++i)
    {
      xetra_15::InsideMarketSnapshotInformation_EntriesDepth_Element& entry = entriesDepth.entries[i];
      switch(entry.entryType())
      {
        case 1: // ASK
        case 2: // BID
  ...
  template<typename T>
  void XETRA_CHannel_Handler_15::apply_snapshot(T& msg, Xetra_Security_Definition_15& sec_def)
  {
    uint64_t entryTime = adjust_to_london_time_15(msg.entryTime());
    Time quotim = Time::today() + Time_Duration(static_cast<int64_t>(entryTime) * 1000); // 

    clear_book(sec_def, quotim);

    FAST_Sequence<xetra_15::InsideMarketSnapshotInformation_EntriesAtp_Element, 50>& entriesAtp = msg.EntriesAtp(); // ToDo: add in typedefs so we can refer generally to T's entries atp type
    for(size_t i = 0; i < entriesAtp.len; ++i)
    {
      xetra_15::InsideMarketSnapshotInformation_EntriesAtp_Element& entry = entriesAtp.entries[i];
      switch(entry.entryType())
      {
        case 6: //LAST_AUCTION
          {
...
*/
  ...
  size_t XETRA_CHannel_Handler_15::handle_snapshot(size_t context, uint8_t* buf, size_t len)
  {
...
*/
            ...
            if(isix < MAX_XETRA_ISIX)
            {
              ...
              { 
                ...
                //show_snapshot<xetra_15::InsideMarketSnapshotInformation>(snapshot, sec_def);
                if(consolSeqNum >= sec_def.expected_seq_num)
                {
                  ...
                  apply_snapshot<xetra_15::InsideMarketSnapshotInformation>(snapshot, sec_def);
                  ...
                  {
                    DeltaMessage15* delta_msg = sec_def.delta_message_queue.Front();
                    ...
                    switch(delta_msg->template_id)
                    {
                      case 7:
                        {
                          xetra_15::InsideMarketDeltaInformation& delta = m_fast_decoder.Parse_InsideMarketDeltaInformation(delta_buf_ptr);
  ...
  size_t XETRA_Handler_15::handle_instrument(uint8_t* buf, size_t len)
  {
    uint8_t* buf_ptr = buf;
    uint8_t* end_ptr = buf + len;
    ...
    if(packet_seq_num < m_instrument_expected_packet_seq_num)
    {
      ...
      {
        m_logger->log_printf(Log::WARN, "XETRA_Handler_15::handle_instrument: packet_seq_num = 1");
      }
      else
      {
        return len;
    ...
    while(buf_ptr < end_ptr)
    {
      ...
      { 
        case 3:
          {
            ...
            {
              xetra_15::InstrumentReferenceData& instrument_reference_data = m_fast_instrument_decoder.Parse_InstrumentReferenceData(buf_ptr);
              ...
              size_t max_depth = 0;
              FAST_Sequence<xetra_15::InstrumentReferenceData_MDFeedTypes_Element, 50>& feedTypes = instrument_reference_data.MDFeedTypes();
              for(size_t i = 0; i < feedTypes.len; ++i)
              {
                xetra_15::InstrumentReferenceData_MDFeedTypes_Element& entry = feedTypes.entries[i];
              ...
              FAST_Sequence<xetra_15::InstrumentReferenceData_SecListGroup_Element, 50>& secList = instrument_reference_data.SecListGroup();
              for(size_t i = 0; i < secList.len; ++i)
              {
                xetra_15::InstrumentReferenceData_SecListGroup_Element& secDef = secList.entries[i];
		...
		LOGV(cout << "  isix=" << isix << endl);
                ...
                if(isix > MAX_XETRA_ISIX)
                {
                  LOGV(cout << "XETRA_CHannel_Handler_15::handle_instrument: got isix " << isix << " which exceeds MAX_XETRA_ISIX " << MAX_XETRA_ISIX << endl);
                }
                Xetra_Security_Definition_15& sec_def = m_security_definitions_by_isix[isix];
  ...
  size_t XETRA_CHannel_Handler_15::handle_trades(size_t context, uint8_t* buf, size_t len)
  {
    uint8_t* buf_ptr = buf;
    uint8_t* end_ptr = buf + len;
        ...
        case 9:
          {
            while(buf_ptr < end_ptr)
            {
              xetra_15::AllTradePrice& msg = m_fast_decoder.Parse_AllTradePrice(buf_ptr);
                ...
                { 
                  ...
                  switch(entryType)
                  {
                    ...
                    uint32_t tranMtchIdNo = msg.tranMtchIdNo();
                      ...
                      uint64_t entryTime = adjust_to_london_time_15(msg.timeStamp());
                      Time exch_time = Time::today() + Time_Duration(static_cast<int64_t>(entryTime) * 1000);                   
