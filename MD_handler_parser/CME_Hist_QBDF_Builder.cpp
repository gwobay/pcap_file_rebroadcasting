...
#include "CME_Hist_QBDF_Builder.h"
...
namespace hf_core {
  ...
  class CME_Rec_Event_Sequence : public Event_Sequence
  {
  public:
    CME_Rec_Event_Sequence(boost::iostreams::filtering_stream<boost::iostreams::input>* fp_stream, string symbol_base,
                           CME_MDP_FAST_Channel_Handler* channel_handler, Time cur_date, hash_map<int, bool>& security_ids, QBDF_Builder* qbdf_builder, 
                           bool is_for_sec_defs_only, Time_Duration time_offset)
      : m_infile(fp_stream), 
...
#endif
      m_utc_offset = Time::today() - Time::utc_today() - (hours(24) - seconds(day_diff.total_sec()));
#ifdef CW_DEBUG
    ...
    bool process_current_message()
    {
      ...
      if(msg_type == "d") // Security Definition
      {
        ...
        QBDF_Builder* cur_builder = m_channel_handler->get_qbdf_builder() ;
        ...
        m_channel_handler->AddInstrumentDefinition(security_id, symbol, maturity_month_year, display_factor, min_price_increment,
                                                  unit_of_measure_qty, outright_market_depth, implied_market_depth, 0, security_group, false);
        m_channel_handler->set_qbdf_builder(cur_builder);
      }
...
*/
          ...
          m_fix_field_parser.get_int(FIX_Field::TradeVolume, raw_group, &trade_volume);
          m_fix_field_parser.get_char(FIX_Field::TradeCondition, raw_group, &trade_condition);
          m_fix_field_parser.get_char(FIX_Field::QuoteCondition, raw_group, &quote_condition);
          m_fix_field_parser.get_char(FIX_Field::MatchEventIndicator, raw_group, &match_event_indicator);
/*
...
*/
          ...
          m_channel_handler->apply_incremental_update(security_id, static_cast<uint32_t>(update_action), entry_type, static_cast<uint32_t>(price_level),
                                                      price, static_cast<int32_t>(size), static_cast<uint32_t>(trade_volume), trade_condition, 
                                                      quote_condition, last_in_group, rpt_seq, match_event_indicator);
        }
      }
      ...
      {
#ifdef CW_DEBUG
        cout << "CME_Hist_QBDF_Builder: Unrecognized msg_type " << msg_type << endl;
#endif
      }
      return true;
    }
    ...
    virtual bool next(Event_Hub& hub)
    {
      ...
      {
#ifdef CW_DEBUG
        cout << Time::current_time().to_string() << " - SOD - setting qbdf builder. m_symbol_base=" << m_symbol_base << endl;
#endif
        m_channel_handler->set_qbdf_builder(m_qbdf_builder);
...
*/
      ...
      if(given_time >= Time::current_time())
      {
  ...
  void get_this_month_files(const string& file_name, list<boost::filesystem::directory_entry>& result)
  {
  ...
  void get_this_month_and_last_month_files(const string& file_name, list<boost::filesystem::directory_entry>& result, boost::gregorian::date cur_day)
  {
    get_this_month_files(file_name, result);
  ...
  int CME_Hist_QBDF_Builder::process_raw_file(const string& file_name)
  {
    Time::set_simulation_mode(true);
    //Time::set_current_time(Time(m_date, "%Y%m%d"));
    ExchangeID exchange_id = m_hf_exch_ids[0];
   
    boost::scoped_ptr<CME_MDP_FAST_Handler> message_handler(new CME_MDP_FAST_Handler(m_app));
...
/////////////
    ...
    if(template_file_items[3] == "FUT") 
    {
      ...
      get_this_month_and_last_month_files(file_name, possible_files, d);
    ...
    {
      get_this_month_files(file_name, possible_files);
      // Weekly
    ...
    for(list<boost::filesystem::directory_entry>::iterator it = possible_files.begin(); it != possible_files.end(); ++it)
    {
      if(boost::filesystem::is_regular_file(it->path()))
      {
           ...
           name_items[file_end_index] != file_end_date)
        {
          ...
          {
            if(name_items[0] == "secdef" || name_items[0] == "EnrichMD")
            {
              continue;
            }
            string cur_date_str = name_items[file_start_index];
...
*/
        ...
        if(cur_file_exchange == exchange)
        {
          ...
          ++file_count;
#ifdef CW_DEBUG
          cout << "opening file " << file_count << " " << cur_file_path << " " << cur_symbol_base << endl;
#endif
          ...
          message_handler->my_one_channel_handler()->set_is_hist(true);
          CME_Rec_Event_Sequence* cme_seq = new CME_Rec_Event_Sequence(fp_stream.get(), cur_symbol_base, message_handler->my_one_channel_handler(), midnight, security_ids, this, is_for_sec_defs_only, time_offset);
